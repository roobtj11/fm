...
export default function CompanionImport() {
    ...
    const analyze = async () => {
        ...
        setError('');
        try {
            const response = await fetch('/api/companion-import', {
                method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ imageDataUrl }),
            });
            const body = await response.json().catch(() => ({}));
            if (!response.ok) throw new Error(body.error || 'The screenshot could not be read.');
            setResult(body.result);
        } catch (cause) {
            setError(cause instanceof Error ? cause.message : 'The screenshot could not be read.');
        } finally {
            setBusy(false);
        }
    };
    ...
    return <div className="mx-auto max-w-6xl space-y-6 pb-20">
        ...
        <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
            ...
            <section className="rounded-2xl border border-border bg-bg-card/70 p-5">
                {!result ? <div className="flex min-h-72 flex-col items-center justify-center text-center"><CopyCheck className="h-10 w-10 text-text-muted" /><h2 className="mt-3 font-black text-text-primary">Review before importing</h2><p className="mt-1 max-w-sm text-xs leading-5 text-text-muted">The recognized name, rarity, level, and substats will appear here. Nothing is added automatically.</p></div> : <div className="space-y-5">
                    <div className="flex items-center justify-between"><div><h2 className="text-xl font-black text-text-primary">Recognized companion</h2><p className="text-xs text-text-muted">Confidence {Math.round((result.confidence || 0) * 100)}% · verify every value</p></div>{duplicate && <span className="rounded-full bg-amber-500/15 px-3 py-1 text-xs font-black text-amber-300">Already saved</span>}</div>
                    <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <Field label="Name"><input value={result.name} onChange={e => update({ name: e.target.value })} className={inputClass} /></Field>
                    ...
                    <div className="space-y-2"><div className="flex items-center justify-between"><h3 className="text-xs font-black uppercase text-text-secondary">Secondary stats</h3><button onClick={() => update({ secondaryStats: [...result.secondaryStats, { statId: statIds[0] || 'DamageMulti', value: 0 }] })} className="flex items-center gap-1 text-xs font-bold text-accent-primary"><Plus className="h-3 w-3" />Add stat</button></div>{result.secondaryStats.map((stat, index) => <div key={`${index}-${stat.statId}`} className="grid grid-cols-[1fr_7rem_2rem] gap-2"><select value={stat.statId} onChange={e => updateStat(index, { statId: e.target.value })} className={inputClass}>{statIds.map(id => <option key={id}>{id}</option>)}</select><input type="number" step="0.01" value={stat.value} onChange={e => updateStat(index, { value: Number(e.target.value) })} className={inputClass} /><button onClick={() => update({ secondaryStats: result.secondaryStats.filter((_, i) => i !== index) })} className="rounded-lg text-red-300 hover:bg-red-500/10"><Trash2 className="mx-auto h-4 w-4" /></button></div>)}</div>
