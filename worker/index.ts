...
const profileSync = async (request: Request, env: Env) => {
    ...
    if (request.method === 'GET') {
        ...
        try { return json({ user, state: { profiles: JSON.parse(row.profiles_json), activeProfileId: row.active_profile_id || undefined, updatedAt: row.updated_at } }); }
    ...
    if (request.method === 'PUT') {
        ...
        return json({ user, state: { profiles: body.profiles, activeProfileId: activeProfileId || undefined, updatedAt } });
    }
...
const scannerTraining = async (request: Request, env: Env) => {
    if (request.method === 'GET') {
        const rows = await env.DB.prepare(`SELECT kind, field, region_json AS regionJson, aspect_ratio AS aspectRatio,
            observed_text AS observedText, corrected_value AS correctedValue FROM scanner_training_examples ORDER BY created_at DESC LIMIT 250`).all();
        return json({ examples: rows.results });
    }
    const user = getUser(request);
    ...
    if (request.method === 'POST') {
        ...
        const statements = body.examples.flatMap((raw: unknown) => {
            ...
            return [env.DB.prepare(`INSERT OR IGNORE INTO scanner_training_examples
                (contributor_key, example_id, kind, field, region_json, aspect_ratio, observed_text, corrected_value, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
